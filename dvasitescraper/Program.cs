...
namespace DvaSiteScraper
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var factSheetDataFromFactSheetIndexPage = from fn in factSheetNodes
                                             ...
                                             select new
                                             {
                                                 ...
                                                 FactSheetName = HtmlEntity.DeEntitize(fn.InnerText),
                                                 FactSheetRelativeLink = fn.GetAttributeValue("href","")
                                             };

            var scrapingTasks = from s in factSheetDataFromFactSheetIndexPage
                                let uri = new Uri(new Uri(Properties.Settings.Default.baseUrl), s.FactSheetRelativeLink)
                                select getFactSheetData(s.FactSheetName, s.KeyWord, uri);

            var top10Tasks = scrapingTasks.Take(10);
            
            var results = top10Tasks.AsParallel()
                .Select(r => r.Result);
        ...
        private static async Task<ScrapedFactSheetData> getFactSheetData(string factSheetName, string dvaKeyword, Uri uri)
        {
            ...
            return new ScrapedFactSheetData(factSheetName, dvaKeyword, contentNode);           
        }
